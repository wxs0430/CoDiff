...
def var_all_case_LA_mean(model1, model2, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    return avg_dice
def var_all_case_pancreas_mean(model1, model2, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4):
    with open('./Datasets/pancreas/data_split/test.txt', 'r') as f:
        image_list = f.readlines()
    image_list = ["./Datasets/pancreas/data/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    print('average metric is {}'.format(avg_dice))
    return avg_dice
def var_all_case_pancreas(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4):
