...
def mask_loss(net3_output, img_l, mask_same=None):
    DICE = mask_DiceLoss(nclass=2)
    CE = nn.CrossEntropyLoss(reduction='none')
    img_l = img_l.type(torch.int64)
    if mask_same == None:
        dice_loss = DICE(net3_output, img_l)
        loss_ce = F.cross_entropy(net3_output, img_l)
    else:
        dice_loss = DICE(net3_output, img_l, mask_same)
        loss_ce = (CE(net3_output, img_l) * mask_same).sum() / (mask_same.sum() + 1e-16)
