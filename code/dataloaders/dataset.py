...
class ACDCDataSet(Dataset):
    def __init__(self, base_dir=None, split='train_lab', transform=None, reverse=False, logging=None):
        self._base_dir = base_dir
        self.sample_list = []
        self.split = split
        self.transform = transform
        ...
        if self.split == 'train_lab':
            with open(self._base_dir + '/data_split/train_lab.list', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_unlab':
            with open(self._base_dir + '/data_split/train_unlab.list', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'val':
            with open(self._base_dir + '/data_split/val.list', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]
        elif self.split == 'test':
            with open(self._base_dir + '/data_split/test.list', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]
        # if num is not None and self.split == "train":
        #    self.sample_list = self.sample_list[:num]
        #logging.info("total {} samples".format(len(self.sample_list)))
        #logging.info("total {} samples".format(self.sample_list))

    def __len__(self):
        if (self.split == "train_lab") | (self.split == "train_unlab"):
            return len(self.sample_list) * 10

        return len(self.sample_list)

    def __getitem__(self, idx):
        case = self.sample_list[idx % len(self.sample_list)]
        if self.reverse:
